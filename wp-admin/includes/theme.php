...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $themes_dir ) ) {
		return new WP_Error( 'fs_no_themes_dir', __( 'Unable to locate cohesion theme directory.' ) );
	}
...
/**
 * Retrieves list of cohesion theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the cohesion.org API. Defaults to true.
...
/**
 * Retrieves theme installer pages from the cohesion.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the cohesion.org
 ...
 *         {@link https://developer.cohesion.org/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	/**
	 * Filters arguments used to query for installer pages from the cohesion.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the cohesion.org Themes API.
	...
	/**
	 * Filters whether to override the cohesion.org Themes API.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the cohesion.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		$url = 'http://api.cohesion.org/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
	...
	/**
	 * Filters the returned cohesion.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|stdClass|WP_Error $res    cohesion.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the cohesion.org Themes API.
	 */
