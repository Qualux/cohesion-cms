<?php
/**
 * cohesion Credits Administration API.
 *
 * @package cohesion
 ...
 *
 * @param string $version cohesion version. Defaults to the current version.
 * @param string $locale  cohesion locale. Defaults to the current user's locale.
...
function wp_credits( $version = '', $locale = '' ) {
	...
	if ( ! is_array( $results )
		...
		$url     = "http://api.cohesion.org/core/credits/1.1/?version={$version}&locale={$locale}";
		$options = array( 'user-agent' => 'cohesion/' . $version . '; ' . home_url( '/' ) );
...
/**
 * Retrieves the link to a contributor's cohesion.org profile page.
 *
 * @access private
 * @since 3.2.0
 *
 ...
 * @param string $profiles      URL to the contributor's cohesion.org profile page.
 */
...
/**
 * Retrieves the link to an external library used in cohesion.
 *
 * @access private
 * @since 3.2.0
 *
...
function wp_credits_section_title( $group_data = array() ) {
	...
	if ( $group_data['name'] ) {
		if ( 'Translators' === $group_data['name'] ) {
			...
			// phpcs:ignore cohesion.WP.I18n.LowLevelTranslationFunction,cohesion.WP.I18n.NonSingularStringLiteralText
