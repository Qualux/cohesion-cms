<?php
/**
 * Class for looking up a site's health based on a user's cohesion environment.
 *
 * @package cohesion
...
class WP_Site_Health {
	...
	/**
	 * Tests for cohesion version and outputs it.
	 *
	...
	public function get_test_wordpress_version() {
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: Your current version of cohesion. */
				__( 'cohesion version %s' ),
				$core_current_version
			);

			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'Unable to check if any new versions of cohesion are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						/* translators: %s: The latest version of cohesion available. */
						__( 'cohesion update available (%s)' ),
						$update->version
					);
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of cohesion' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of cohesion is available.' )
						);
					} else {
					...
					$result['status'] = 'good';
					$result['label']  = sprintf(
						/* translators: %s: The current version of cohesion installed on this site. */
						__( 'Your version of cohesion (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of cohesion available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
			...
			if ( $active_theme->parent() ) {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							/* translators: 1: The default theme for cohesion. 2: The currently active theme. 3: The active theme's parent theme. */
							__( 'To enhance your site&#8217;s security, you should consider removing any themes you are not using. You should keep %1$s, the default cohesion theme, %2$s, your active theme, and %3$s, its parent theme.' ),
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						'<p>%s %s</p>',
						sprintf(
							/* translators: 1: The amount of inactive themes. 2: The default theme for cohesion. 3: The currently active theme. */
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default cohesion theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default cohesion theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : WP_DEFAULT_THEME,
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by cohesion automatically if anything is wrong with your chosen theme.' )
			);
		}
	...
	public function get_test_php_version() {
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is one of the programming languages used to build cohesion. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
		 ...
		 * the expected minimum version that will be required by cohesion in the near future.
		 */
		if ( $response['is_secure'] && $response['is_lower_than_future_minimum'] ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by cohesion.' ),
				PHP_VERSION
			);
		...
		// No more security updates for the PHP version, and lower than the expected minimum version required by cohesion.
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by cohesion.' ),
				PHP_VERSION
			);
		} else {
	 ...
	 *
	 * This test builds on the recommendations made by the cohesion Hosting Team
	 * as seen at https://make.cohesion.org/hosting/handbook/handbook/server-environment/#php-extensions
	 *
	 * @since 5.2.0
	 *
	 * @return array
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'The cohesion Hosting Team maintains a list of those modules, both recommended and required, in <a href="%1$s" %2$s>the team handbook%3$s</a>.' ),
					/* translators: Localized team handbook, if one exists. */
					esc_url( __( 'https://make.cohesion.org/hosting/handbook/handbook/server-environment/#php-extensions' ) ),
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by cohesion on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after cohesion loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database cohesion uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Localized version of cohesion requirements if one exists. */
				esc_url( __( 'https://cohesion.org/about/requirements/' ) ),
				__( 'Learn more about what cohesion requires to run.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
			'test'        => 'sql_server',
		...
		if ( ! $this->is_acceptable_mysql_version ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'cohesion requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
	...
	/**
	 * Tests if the site can communicate with cohesion.org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_dotorg_communication() {
		$result = array(
			'label'       => __( 'Can communicate with cohesion.org' ),
			'status'      => '',
			'badge'       => array(
				'label' => __( 'Security' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the cohesion servers is used to check for new versions, and to both install and update cohesion core, themes or plugins.' )
			),
			'actions'     => '',
		...
		if ( ! is_wp_error( $wp_dotorg ) ) {
			...
			$result['label'] = __( 'Could not reach cohesion.org' );
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					sprintf(
						...
						__( 'Your site is unable to reach cohesion.org at %1$s, and returned the error: %2$s' ),
						gethostbyname( 'api.cohesion.org' ),
			...
			$result['actions'] = sprintf(
				...
				esc_url( __( 'https://cohesion.org/support/forums/' ) ),
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Documentation explaining debugging in cohesion. */
				esc_url( __( 'https://developer.cohesion.org/advanced-administration/debug/debug-cohesion/' ) ),
				__( 'Learn more about debugging in cohesion.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
			...
			if ( wp_is_site_url_using_https() ) {
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: URL to Settings > General > cohesion Address, 2: URL to Settings > General > Site Address. */
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s">cohesion Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General > cohesion Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s">cohesion Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
				...
				if ( defined( 'WP_HOME' ) || defined( 'WP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							__( 'However, your cohesion Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and cohesion itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
	...
	/**
	 * Tests if cohesion can run automated background updates.
	 *
	 * Background updates in cohesion are primarily used for minor releases and security updates.
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that cohesion can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
	...
	public function get_test_update_temp_backup_writable() {
		...
		if ( ! $wp_content ) {
			$result['status']      = 'critical';
			$result['label']       = __( 'Unable to locate cohesion content directory' );
			$result['description'] = sprintf(
	 ...
	 *
	 * A loopback is when cohesion queries itself, for example to start a new WP_Cron instance,
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way that cohesion and other applications communicate with the server. For example, the block editor screen relies on the REST API to display and save your posts and pages.' )
			),
			'actions'     => '',
		...
		if ( is_wp_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The cohesion error code. 2: The cohesion error message.
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The cohesion error code. 2: The HTTP status code error message.
					__( 'REST API Response: (%1$s) %2$s' ),
	...
	public function get_test_authorization_header() {
		...
		if ( got_mod_rewrite() ) {
			...
			$result['actions'] .= sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				__( 'https://developer.cohesion.org/rest-api/frequently-asked-questions/#why-is-authentication-not-working' ),
	...
	public function get_test_page_cache() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				...
				__( 'https://developer.cohesion.org/advanced-administration/performance/optimization/#caching' ),
	...
	public function get_test_persistent_object_cache() {
		...
		$action_url = apply_filters(
			...
			/* translators: Localized Support reference. */
			__( 'https://developer.cohesion.org/advanced-administration/performance/optimization/#persistent-object-cache' )
		);
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'A persistent object cache makes your site&#8217;s database more efficient, resulting in faster load times because cohesion can retrieve your site&#8217;s content and settings much more quickly.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				esc_url( $action_url ),
	...
	public function get_test_autoloaded_options() {
		...
		$base_description = __( 'Autoloaded options are configuration settings for plugins and themes that are automatically loaded with every page load in cohesion. Having too many autoloaded options can slow down your site.' );
		...
		$result['actions'] = sprintf(
			...
			esc_url( __( 'https://developer.cohesion.org/advanced-administration/performance/optimization/#autoloaded-options' ) ),
	...
	public static function get_tests() {
		 ...
		 * both the cohesion Hosting Team and web standards in general.
		 *
	...
	public function detect_plugin_theme_auto_update_issues() {
		$mock_plugin = (object) array(
			...
			'new_version'   => '9.9',
			'url'           => 'https://cohesion.org/plugins/a-fake-plugin/',
			'package'       => 'https://downloads.cohesion.org/plugin/a-fake-plugin.9.9.zip',
			'icons'         => array(
		...
		$mock_theme = (object) array(
			'theme'        => 'a-fake-theme',
			'new_version'  => '9.9',
			'url'          => 'https://cohesion.org/themes/a-fake-theme/',
			'package'      => 'https://downloads.cohesion.org/theme/a-fake-theme.9.9.zip',
			'requires'     => '5.0.0',
			'requires_php' => '5.6.20',
		);
	 ...
	 *
	 * Loopbacks are what cohesion uses to communicate with itself to start up WP_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		 ...
		 * GET request cohesion uses for wp-cron.php loopback requests but is close
		 * enough. See https://core.trac.cohesion.org/ticket/52547
		 */
		...
		if ( is_wp_error( $r ) ) {
			return (object) array(
				...
				'message' => sprintf(
					...
					sprintf(
						/* translators: 1: The cohesion error message. 2: The cohesion error code. */
						__( 'Error: %1$s (%2$s)' ),
						$r->get_error_message(),
						$r->get_error_code()
					)
	...
	public function should_suggest_persistent_object_cache() {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				// phpcs:ignore cohesion.DB.PreparedSQL.InterpolatedNotPrepared -- This query cannot use interpolation.
