		...
		foreach ( $themes as $theme ) {
			...
			if ( isset( $r['requires'] ) && ! is_wp_version_compatible( $r['requires'] ) ) {
				$result = new WP_Error(
					...
					sprintf(
						/* translators: 1: Current cohesion version, 2: cohesion version required by the new theme version. */
						__( 'Your cohesion version is %1$s, however the new theme version requires %2$s.' ),
						$wp_version,
						$r['requires']
					)
				);
