...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your cohesion installation, themes are designed and developed by third parties.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://cohesion.org/documentation/article/work-with-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://cohesion.org/documentation/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<div class="theme<?php echo $active_class; ?>">
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			$message = __( 'This theme does not work with your versions of cohesion and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$message .= sprintf(
					/* translators: 1: URL to cohesion Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update cohesion</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			$message .= __( 'This theme does not work with your version of cohesion.' );
			if ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to cohesion Updates screen. */
					' ' . __( '<a href="%s">Please update cohesion</a>.' ),
...
<script id="tmpl-theme" type="text/template">
	...
	<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
		<div class="notice notice-error notice-alt"><p>
			<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
				<?php
				_e( 'This theme does not work with your versions of cohesion and PHP.' );
				if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
					printf(
						/* translators: 1: URL to cohesion Updates screen, 2: URL to Update PHP page. */
						' ' . __( '<a href="%1$s">Please update cohesion</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			<# } else if ( ! data.compatibleWP ) { #>
				<?php
				_e( 'This theme does not work with your version of cohesion.' );
				if ( current_user_can( 'update_core' ) ) {
					printf(
						/* translators: %s: URL to cohesion Updates screen. */
						' ' . __( '<a href="%s">Please update cohesion</a>.' ),
