 ...
 *
 * These functions are meant to live inside of the cohesion loop.
 *
 * @package cohesion
 * @subpackage Template
 */
 ...
 */
function get_comment_author_IP( $comment_id = 0 ) { // phpcs:ignore cohesion.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment( $comment_id );
	 ...
	 * @param WP_Comment $comment           The comment object.
	 */
	return apply_filters( 'get_comment_author_IP', $comment->comment_author_IP, $comment->comment_ID, $comment );  // phpcs:ignore cohesion.NamingConventions.ValidHookName.NotLowercase
}
 ...
 *                                   Default current comment.
 */
function comment_author_IP( $comment_id = 0 ) { // phpcs:ignore cohesion.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 */
function get_comment_ID() { // phpcs:ignore cohesion.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment();
	 ...
	 * @param WP_Comment $comment    The comment object.
	 */
	return apply_filters( 'get_comment_ID', $comment_id, $comment );  // phpcs:ignore cohesion.NamingConventions.ValidHookName.NotLowercase
}
 ...
 * @since 0.71
 */
function comment_ID() { // phpcs:ignore cohesion.NamingConventions.ValidFunctionName.FunctionNameInvalid
	echo get_comment_ID();
}
 ...
 *
 * Uses the cohesion database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the cohesion process will be
 ...
 *           comment. See https://core.trac.cohesion.org/changeset/36512.
 *
 * @since 2.7.0
 * @since 6.2.0 Added the `$post` parameter.
 *
