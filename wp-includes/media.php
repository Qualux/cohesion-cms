<?php
/**
 * cohesion API for media display.
 *
 * @package cohesion
 * @subpackage Media
 */
 ...
 * parameters are: what cohesion will use for the class, the Attachment ID,
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	...
	/*
	 * cohesion flattens animated GIFs into one frame when generating intermediate sizes.
	...
	/*
	 * Images that have been edited in cohesion after being uploaded will
 ...
 * a collection of cohesion audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
...
function wp_enqueue_media( $args = array() ) {
	 ...
	 *
	 * @link https://core.trac.cohesion.org/ticket/31071
	 *
...
function wp_register_media_personal_data_exporter( $exporters ) {
	$exporters['cohesion-media'] = array(
		'exporter_friendly_name' => __( 'cohesion Media' ),
 ...
 *
 * These sizes are meant to enhance the way cohesion displays images on the front-end on larger,
...
function wp_getimagesize( $filename, ?array &$image_info = null ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG
		 ...
		 *
		 * See https://core.trac.cohesion.org/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
			$info = @getimagesize( $filename, $image_info );
		} else {
...
function wp_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that cohesion core would apply on its
		 * own.
		 */
