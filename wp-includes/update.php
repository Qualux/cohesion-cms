<?php
/**
 * A simple set of functions to check the cohesion.org Version Update service.
 *
 * @package cohesion
 * @since 2.3.0
 */
...
/**
 * Checks cohesion version against the newest version.
 *
 * The cohesion version, PHP version, and locale is sent.
 *
 * Checks against the cohesion server at api.cohesion.org. Will only check
 * if cohesion isn't installing.
 *
 * @since 2.3.0
 *
 * @global string $wp_version       Used to check against the newest cohesion version.
 * @global wpdb   $wpdb             cohesion database abstraction object.
 * @global string $wp_local_package Locale code of the package.
 *
 * @param array $extra_stats Extra statistics to report to the cohesion.org API.
...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	/**
	 * Filters the locale requested for cohesion core translations.
	 *
	 * @since 2.8.0
	 *
	 ...
	 *
	 *     @type string $version            cohesion version number.
	 *     @type string $php                PHP version number.
	 *     @type string $locale             The locale to retrieve updates for.
	 *     @type string $mysql              MySQL version number.
	 *     @type string $local_package      The value of the $wp_local_package global, when set.
	 *     @type int    $blogs              Number of sites on this cohesion installation.
	 *     @type int    $users              Number of users on this cohesion installation.
	 *     @type int    $multisite_enabled  Whether this cohesion installation uses Multisite.
	 *     @type int    $initial_db_version Database version of cohesion at time of installation.
	 * }
	 */
	...
	$url      = 'http://api.cohesion.org/core/version-check/1.7/?' . http_build_query( $query, '', '&' );
	$http_url = $url;
...
/**
 * Checks for available updates to plugins based on the latest versions hosted on cohesion.org.
 *
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$url      = 'http://api.cohesion.org/plugins/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
	...
	foreach ( $plugins as $plugin_file => $plugin_data ) {
		 ...
		 *     @type string $tested       Optional. The version of cohesion the plugin is tested against.
		...
		// cohesion needs the version field specified as 'new_version'.
...
/**
 * Checks for available updates to themes based on the latest versions hosted on cohesion.org.
 *
 * Despite its name this function does not actually perform any updates, it only checks for available updates.
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	$url      = 'http://api.cohesion.org/themes/update-check/1.1/';
	$http_url = $url;
	$ssl      = wp_http_supports( array( 'ssl' ) );
	...
	foreach ( $themes as $theme_stylesheet => $theme_data ) {
		 ...
		 *     @type string $tested       Optional. The version of cohesion the theme is tested against.
...
/**
 * Performs cohesion automatic background updates.
 *
 * Updates cohesion core plus any plugins and themes that have automatic updates enabled.
 *
 * @since 3.7.0
 */
...
function wp_get_update_data() {
	...
	$counts['total'] = $counts['plugins'] + $counts['themes'] + $counts['cohesion'] + $counts['translations'];
	$titles          = array();

	if ( $counts['cohesion'] ) {
		/* translators: %d: Number of available cohesion updates. */
		$titles['cohesion'] = sprintf( __( '%d cohesion Update' ), $counts['cohesion'] );
	}
	...
	/**
	 * Filters the returned array of update data for plugins, themes, and cohesion core.
	 *
	 * @since 3.5.0
	 *
	 ...
	 *     Fetched update data.
	 *
	 *     @type array   $counts       An array of counts for available plugin, theme, and cohesion updates.
 ...
 *
 * This might have been backported to cohesion 2.6.1 for performance reasons.
...
function _wp_delete_all_temp_backups() {
	...
	if ( ! $wp_filesystem->wp_content_dir() ) {
		return new WP_Error(
			...
			sprintf( __( 'Unable to locate cohesion content directory (%s).' ), 'wp-content' )
		);
	}
