...
abstract class Controls_Stack extends Base_Object {
	...
	final public function get_position_info( array $position ) {
		...
		if (
			...
			_doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), 'Invalid position arguments. Use `before` / `after` for control or `start` / `end` for section.', '1.7.0' ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
	...
	final public function add_group_control( $group_name, array $args = [], array $options = [] ) {
		...
		if ( ! $group ) {
			wp_die( sprintf( '%s::%s: Group "%s" not found.', get_called_class(), __FUNCTION__, $group_name ) ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function start_controls_section( $section_id, array $args = [] ) {
		...
		if ( null !== $this->current_section ) {
			wp_die( sprintf( 'Elementor: You can\'t start a section before the end of the previous section "%s".', $this->current_section['section'] ) ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function start_controls_tabs( $tabs_id, array $args = [] ) {
		if ( null !== $this->current_tab ) {
			wp_die( sprintf( 'Elementor: You can\'t start tabs before the end of the previous tabs "%s".', $this->current_tab['tabs_wrapper'] ) ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function start_controls_tab( $tab_id, $args ) {
		if ( ! empty( $this->current_tab['inner_tab'] ) ) {
			wp_die( sprintf( 'Elementor: You can\'t start a tab before the end of the previous tab "%s".', $this->current_tab['inner_tab'] ) ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
		}
	...
	protected function print_template_content( $template_content ) {
		echo $template_content; // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
	}
	...
	protected function handle_control_position( array $args, $control_id, $overwrite ) {
		...
		if ( null !== $target_section_args ) {
			if ( ! empty( $args['section'] ) || ! empty( $args['tab'] ) ) {
				_doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( 'Cannot redeclare control with `tab` or `section` args inside section "%s".', $control_id ), '1.0.0' ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( ! Performance::should_optimize_controls() ) {
				wp_die( sprintf( '%s::%s: Cannot add a control outside of a section (use `start_controls_section`).', get_called_class(), __FUNCTION__ ) ); // phpcs:ignore cohesion.Security.EscapeOutput.OutputNotEscaped
			}
		}
